...
export async function confirmReceiptExpense(
  ...
  if (batchMode) {
    ...
    if (decision.action === "empty") {
      throw new Error("Inga SMS att spara från den här bilden");
    }
    ...
    if (!lastRow) throw new Error("Kunde inte spara SMS");
